...
const ChatBot = () => {
  ...
  const formatTableMessage = (text: string) => {
    if (text.includes('|') && text.includes('---')) {
      ...
      const processedRows = tableRows.map((row, rowIndex) => {
        ...
        const isScoreLine = /^(\d+(\.\d+)?)$/.test(cells[0]) || 
                           /^(\d+)[-–](\d+(\.\d+)?)$/.test(cells[0]);
        ...
        <div className="w-full">
          ...
          <div className="overflow-x-auto mb-3">
            <table className="w-full border-collapse rounded-md">
              <thead>
                {processedRows.filter(row => row.isHeader).map((row, rowIndex) => (
              ...
              <tbody>
                {processedRows.filter(row => !row.isHeader && !row.cells.some(cell => cell.includes('---'))).map((row, rowIndex) => {
  ...
  const formatMessage = (text: string) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if ((line.includes('hài lòng') && (line.includes('Câu trả lời') || line.includes('còn câu hỏi'))) || 
      ...
      >
        ...
        {messages.map((message, index) => (
          <div key={index} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'} gap-2`}>
            {message.sender === 'bot' && (
              <div className={`${styles.avatar} bg-primary/10 p-2 rounded-full flex-shrink-0 self-start h-9 w-9 flex items-center justify-center`}>
            ...
            >
              {message.sender === 'user' ? (
                ...
                (isMessageViewed(message) ? 
                  <div className='text-sm whitespace-pre-wrap' dangerouslySetInnerHTML={{ __html: formatMessage(message.text) }}></div> : 
