...
export default function DialogAnswerSuggestions({
  answers,
  onSelectAnswer,
  isLoading
}: DialogAnswerSuggestionsProps) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="secondary" type="button" className="flex items-center gap-1">
          ...
          {answers.length > 0 && <span className="text-xs bg-primary text-white rounded-full px-2">{answers.length}</span>}
        </Button>
      </DialogTrigger>
